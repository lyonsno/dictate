...
class SpokeAppDelegate(NSObject):
    ...
    def init(self):
        ...
        # Command pathway — initialized if a local/sidecar URL can be resolved.
        ...
        if selection is None:
            ...
            if self._command_client is not None:
                state["assistant"] = {
                    ...
                    "models": getattr(self, "_command_model_options", []),
                }
    ...
    def _resolve_command_backend(self) -> tuple[str | None, str | None]:
        env_url = self._normalize_command_url(os.environ.get("SPOKE_COMMAND_URL"))
        pref_backend = self._load_command_backend_preference()
        pref_sidecar_url = self._load_command_sidecar_url_preference()
        explicit_env_url = env_url and env_url != _DEFAULT_COMMAND_URL
        ...
        if (
            pref_backend is not None
            or pref_sidecar_url is not None
            or self._load_command_model_preference() is not None
    ...
    def _apply_command_backend_selection(self, selection: str) -> None:
        ...
        if not self._save_command_backend_preferences(selection, persisted_sidecar_url):
            logger.warning(
