    ...
    def _make_tool_executor(self):
        ...
        def _write_compaction_trace(event: str, **payload) -> None:
            ...
            try:
                ...
                logger.debug("compact_history trace write failed", exc_info=True)

        def _drop_persisted_tool_results(turn):
        ...
        def _compact_history(arguments):
            ...
            history = getattr(client, "_history", None) if client is not None else None
            if not history:
                return json.dumps({"status": "nothing to compact", "turns": 0})
            ...
            if mode == "drop_tool_results":
                ...
                for index in range(target):
                    turn = history[index]
                    cleaned = _drop_persisted_tool_results(turn)
                    if cleaned != turn:
            ...
            if mode == "summarize":
                ...
                if not isinstance(summary, str) or not summary:
                    return json.dumps(
                        {"error": "summary is required for summarize mode"}
                    )
                remaining = history[target:]
                summary_turn = ("[compacted history]", summary)
                ...
                return json.dumps(
                    {
                        ...
                        "mode": "guided",
                        "error": (
                            "guided compact_history requires the Converge retrieval "
                            "carry; this hotfix restores tool availability for "
                            "drop_tool_results and summarize only"
                        ),
                    }
                )
