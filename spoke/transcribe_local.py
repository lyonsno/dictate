...
class LocalTranscriptionClient:
    ...
    def transcribe(self, wav_bytes: bytes) -> str:
        ...
        if supports_eager_eval():
            # When the user explicitly enables eager_eval, do not also forward
            # Spoke's default timeout guard. The backend's timeout path already
            # forces synchronous step evaluation, which would otherwise make
            # the stability-mode toggle a no-op. Explicit non-default timeout
            # choices still take precedence and are forwarded.
            if not (
