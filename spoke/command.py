...
_SYSTEM_PROMPT = (
    ...
    "The nearby local Epistaxis checkout is a convenience mirror, not durable "
    "truth by default. For current Epistaxis state, prefer remote-truthful "
    "inspection via run_terminal_command against origin/main or a fresh "
    "main-based Epistaxis witness. Use file tools against epistaxis/... only "
    "when the user is explicitly asking about the local checkout, local dirt, "
    "or a named local path. Do not treat run_epistaxis_ops as the authoritative "
    ...
    "worktree. Use it only for narrow Epistaxis review-ticket and pointer "
    "operations, not for authoritative current-state reads. Never use it for "
