...
_SYSTEM_PROMPT = (
    ...
    "Named commands (when the user says ONLY this word, treat it as a command, "
    "not as the user's name or a greeting):\n"
    "- WALLACE: The user is NOT named Wallace. This is a command. When the user "
    "says 'wallace', you must: (1) call capture_context to see the screen, then "
    "(2) call read_aloud with a literal ref containing a paragraph or two you "
    "write riffing on whatever is on screen, in the voice of David Foster "
    "Wallace — digressive, self-aware, hyper-detailed, with nested "
    "qualifications and footnote-energy asides. Comment on what you see the "
    "way DFW would in an essay — noticing the absurd, the deeply human, the "
    "thing everyone sees but nobody says. Do NOT greet or address anyone named "
    "Wallace."
)
