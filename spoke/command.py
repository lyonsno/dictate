...
_SYSTEM_PROMPT = (
    ...
    "When the user references epistaxis state, use file tools or bounded terminal reads. "
    "Do not assume a special Epistaxis mutation helper is available.\n"
    "Epistaxis git operations (merge, push, branch cleanup, resolving "
    "divergence, syncing main): the Epistaxis repo has a two-checkout "
    "layout with specific conventions. If you attempt a git command in "
    "the Epistaxis repo and the runbook has not been loaded yet, the "
    "tool will return the runbook instead of executing the command. "
    "Read it carefully and follow its instructions. A merge is not "
    "complete until it has been pushed to origin.\n\n"
    ...
    "oldest N turns with your summary), guided (return attractor-aware "
    "retention flags for the oldest N turns, then follow up with summarize "
    "using those flags as a safety net), and reset_to_summary (drop all "
    "turns after the most recent compaction summary, keeping only the "
    "summary — use when post-compaction conversation has gone off the "
    "rails and you want to restart from a clean known state).\n\n"
