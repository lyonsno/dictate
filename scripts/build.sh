#!/usr/bin/env bash
# Build Spoke.app with PyInstaller and sign for macOS.
set -euo pipefail
...
if [ "${1:-}" = "--fast" ]; then
    ...
    rm -rf build "$APP_PATH" "$DIST_DIR/Spoke"
    find "$PROJECT_DIR/spoke" -type d -name "__pycache__" -exec rm -rf {} + 2>/dev/null || true
    CLEAN_FLAG="--clean"
fi

echo "==> Running PyInstaller..."
uv run pyinstaller Spoke.spec $CLEAN_FLAG --noconfirm
