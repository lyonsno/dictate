#!/usr/bin/env bash
# Build DontType.app with PyInstaller and sign for macOS.
set -euo pipefail
...
if [ "${1:-}" = "--fast" ]; then
    ...
    rm -rf build "$APP_PATH" "$DIST_DIR/DontType"
    find "$PROJECT_DIR/donttype" -type d -name "__pycache__" -exec rm -rf {} + 2>/dev/null || true
    CLEAN_FLAG="--clean"
fi
...
if [ "${1:-}" = "--fast" ]; then
    BUNDLE_ID="com.noahlyons.donttype"
