...
KOKORO_EN_CORE_WEB_SM_URL="https://github.com/explosion/spacy-models/releases/download/en_core_web_sm-3.8.0/en_core_web_sm-3.8.0-py3-none-any.whl"
...
ensure_pip_available() {
    ...
    if ! "$python_bin" -m pip --version >/dev/null 2>&1; then
        echo "==> Bootstrapping pip into fork venv..."
        "$python_bin" -m ensurepip --upgrade
    fi
}

ensure_kokoro_runtime_deps() {
    local python_bin="$1"
    ensure_pip_available "$python_bin"
    if ! "$python_bin" -c "import en_core_web_sm" >/dev/null 2>&1; then
        echo "==> Installing Kokoro English spaCy model..."
        "$python_bin" -m pip install "$KOKORO_EN_CORE_WEB_SM_URL"
    fi
}
...
main() {
    ...
    ensure_kokoro_runtime_deps "$VENV_PYTHON"
