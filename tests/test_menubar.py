...
class TestMenuBarIcon:
    ...
    def test_build_menu_shows_vision_quest_submenu(self, menubar_module):
        """Vision Quest should expose the Tintilla panel opener from the menubar."""
        AppKit = __import__("AppKit")
        ...
        calls = AppKit.NSMenuItem.alloc.return_value.initWithTitle_action_keyEquivalent_.call_args_list
        assert any(call.args == ("Vision Quest", None, "") for call in calls)
        assert any(call.args == ("Show Tintilla Panel", "selectModel:", "") for call in calls)
