...
class TestCommandOverlayToggle:
    ...
    def test_toggle_command_overlay_restores_collapsed_thinking_before_streaming_text(
        self, main_module, monkeypatch
    ):
        d = _make_delegate(main_module, monkeypatch)
        d._command_client = MagicMock()
        ...
        d._command_overlay.show.assert_called_once_with(preserve_thinking_timer=True)
        d._command_overlay.set_utterance.assert_called_once_with("open file")
        d._command_overlay.set_thinking_collapsed.assert_called_once_with(
            "Thought for 3s · reading config structure"
        )
