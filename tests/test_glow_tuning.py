...
=======
    ...
    def test_distance_warp_biases_wide_bloom_tail_more_than_the_edge(self, mock_pyobjc):
        """Temporal wide-bloom drift should preserve the monitor edge while moving the outer shell."""
        import numpy as np

        sys.modules.pop("spoke.glow", None)
        mod = importlib.import_module("spoke.glow")
        try:
            signed_distance = np.array([[-2.0, -18.0]], dtype=np.float32)
            distance_warp = np.array([[4.0, 4.0]], dtype=np.float32)
            ...
            warped = mod._distance_field_alpha(
                signed_distance,
                18.0,
                2.0,
                intensity_multiplier=1.0,
            ...
            edge_drop = float(baseline[0, 0] - warped[0, 0])
            tail_drop = float(baseline[0, 1] - warped[0, 1])

            assert tail_drop > edge_drop
            assert warped[0, 0] > 0.9
        finally:
            sys.modules.pop("spoke.glow", None)

    def test_composited_distance_field_image_can_reuse_cached_signed_distance(self, mock_pyobjc, monkeypatch):
        """Per-frame drift should be able to reuse the cached display SDF instead of regenerating geometry."""
        import numpy as np

        sys.modules.pop("spoke.glow", None)
        mod = importlib.import_module("spoke.glow")
        try:
            signed_distance = np.array([[-4.0]], dtype=np.float32)
            monkeypatch.setattr(
                mod,
                "_display_signed_distance_field",
                lambda geometry: (_ for _ in ()).throw(AssertionError("should use cached signed distance")),
            )
            monkeypatch.setattr(mod, "_premultiplied_rgba_to_image", lambda rgba: ("image", rgba))

            image, payload = mod._composited_distance_field_image(
                {"scale": 1.0},
                [{"name": "wide_bloom", "falloff": 10.0, "power": 2.0}],
                lambda spec: ((1.0, 1.0, 1.0), 1.0),
                signed_distance=signed_distance,
            )

            assert image == "image"
            assert payload.shape == (1, 1, 4)
        finally:
            sys.modules.pop("spoke.glow", None)

    def test_wide_bloom_noise_timer_targets_preview_frame_cadence(self, mock_pyobjc):
        """Vision Quest drift should tick at frame cadence instead of a coarse sampling timer."""
        sys.modules.pop("spoke.glow", None)
        mod = importlib.import_module("spoke.glow")
        try:
            glow = self._make_glow(mod)
            glow._visible = True
            glow._active_wide_bloom_noise_mode = mod.WIDE_BLOOM_NOISE_MODE_DRIFT
            glow._wide_bloom_noise_timer = None

            timer = MagicMock()
            mod.NSTimer.scheduledTimerWithTimeInterval_target_selector_userInfo_repeats_.return_value = timer

            glow._sync_wide_bloom_noise_timer()

            call = mod.NSTimer.scheduledTimerWithTimeInterval_target_selector_userInfo_repeats_.call_args
            assert call.args[0] == pytest.approx(1.0 / 60.0)
            assert glow._wide_bloom_noise_timer is timer
        finally:
            sys.modules.pop("spoke.glow", None)
