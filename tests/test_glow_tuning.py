...
class TestGlowTuning:
    ...
    def test_show_applies_brightness_adaptive_glow_style(self, mock_pyobjc, monkeypatch):
        ...
        # Mock dependencies on the module
        monkeypatch.setattr(mod, "NSTimer", MagicMock())
        monkeypatch.setattr(mod, "CABasicAnimation", MagicMock())
        monkeypatch.setattr(mod, "CAMediaTimingFunction", MagicMock())
        
        glow = self._make_glow(mod)
        monkeypatch.setattr(mod, "_sample_screen_brightness", lambda screen: [1.0] * 16)
    ...
    def test_show_applies_dark_background_glow_style(self, mock_pyobjc, monkeypatch):
        ...
        glow = self._make_glow(mod)
        monkeypatch.setattr(mod, "_sample_screen_brightness", lambda screen: [0.0] * 16)
    ...
    def test_show_invalidates_pending_hide_timer(self, mock_pyobjc, monkeypatch):
        ...
        monkeypatch.setattr(mod, "_sample_screen_brightness", lambda screen: [0.5] * 16)
    ...
    def test_hide_schedules_window_teardown_after_dim_fade(self, mock_pyobjc, monkeypatch):
        """The window should stay alive until the dim fade has had time to finish."""
        mock_timer_class = MagicMock()
        monkeypatch.setattr(mod, "NSTimer", mock_timer_class)
        monkeypatch.setattr(mod, "CABasicAnimation", MagicMock())
        monkeypatch.setattr(mod, "CAMediaTimingFunction", MagicMock())
        ...
        timer = MagicMock()
        mock_timer_class.scheduledTimerWithTimeInterval_target_selector_userInfo_repeats_.return_value = timer
...
>>>>>>> c860ec7
        ...
        anim = mock_anim_class.animationWithKeyPath_.return_value
