...
- **CRITICAL WORKTREE BOOTSTRAP:** If you add a new worktree to the registry that lives outside the main `/Users/noahlyons/dev/spoke` repo (e.g., in a temp directory), **you MUST run `uv sync --extra tts --group dev` inside that worktree's directory before asking the user to test it**. The launch script relies on finding an active `.venv` at the target path, and without this step, it will silently fail to launch.
