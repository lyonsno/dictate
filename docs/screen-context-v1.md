...
2. app extracts OCR + window metadata (+ AX hints when available)
...
Accessibility data is best-effort. If cheap AX hints are available, include
them:
...
AX queries must not block capture. Use a hard timeout and fall back to
OCR-only if AX is slow or returns garbage (common in Electron apps and
complex web views). The capture must always succeed without AX.
...
locally under app support or a local cache directory. The assistant should not
need to know the file path.

### Capture lifetime

Scene captures are bounded by prefill budget, not by wall-clock time. The
operational limit is how much scene context the assistant can hold without
degrading response quality or latency.

V1 should retain a fixed number of recent captures (exact cap TBD during
implementation) and evict oldest-first when the cap is reached. Temporal
pruning (e.g., discard after a day or a week) is a hygiene concern, not an
operational one — add it later to avoid surfacing stale context the user has
forgotten about, but do not use it as the primary eviction policy.
...
The `summary` field is generated by a small local vision model (e.g., Qwen 3.5
0.6B) running a single inference pass on a ~1/4 res downsampled capture. This
is the only unstructured extraction that requires the local vision model — OCR
and metadata come from dedicated extractors. At 0.6B parameters and reduced
resolution, the cost of one summary pass per capture is negligible.
...
Note: V1 intentionally combines ref resolution and TTS dispatch into a single
`read_aloud` call. These are logically separate steps — resolution maps a ref
to text, dispatch sends text to Voxtral — and a future version will likely
split them to support non-speech uses (e.g., "show me what that says"). For V1,
the combined call is simpler and sufficient.
...
When multiple OCR blocks are plausible matches for a referent, default to the
highest-confidence match. Do not pre-design disambiguation flows — address
failure modes reactively based on real usage patterns.

Partial OCR (e.g., a paragraph split across multiple blocks with gaps) is a
known possible failure mode. Modern OCR models handle this well enough that V1
should not pre-design around it. Evaluate on real captures during
implementation and address specific failure patterns as they emerge.
...
- AX hints: best-effort, hard timeout, never blocks capture
- summary: single local vision model pass at ~1/4 res
